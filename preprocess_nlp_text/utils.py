...
def _get_charcounts(x):
    s = x.split()
    join_no_space = ''.join(s)
    return len(join_no_space)
...
def _get_stopwords(x):
    return len([text for text in x.split() if text in stopwords])
    
def _get_hashtag_counts(x):
    return len([i for i in x.split() if i.startswith("#") ])

def _get_mentions_counts(x):
    return len([i for i in x.split() if i.startswith("@") ])

def _get_digit_counts(x):
    return len([text for text in x.split() if text.isdigit()])
